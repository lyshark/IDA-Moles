...
This project was developed by an individual using nearly a month of spare time and leisure. It is positioned as a functional rapid verification prototype. Currently, there is still considerable room for optimization and improvement in the code. Developers are welcome to exchange ideas, discuss, submit pull requests (PRs), and collaborate on iterative development.

Currently, the IDA plugin ecosystem is primarily implemented using IDAPython, and most existing native C++ plugins are developed based on IDA 6.x and 7.x. From version 6.x to 9.1, the SDK interface underwent significant refactoring, resulting in a serious compatibility gap between old and new versions. Additionally, some parts of the official documentation contain deviations and omissions. IDA 9.1 was officially released on February 28, 2025, and currently, there are very few directly reusable practical cases available; the official documentation only lists basic interface prototypes, lacking real-world implementation examples. It is difficult to develop stable and usable functions solely relying on the documentation.

The value of this plugin's source code lies precisely in this: all encapsulated interfaces can operate stably and are unlikely to cause IDA to crash. It serves as an excellent reference and learning case for researching the development of native C++ plugins for IDA 9.1.
