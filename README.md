...
**Author's Note:** This project was developed by an individual over nearly a month using personal time and spare hours, serving as a rapid functional verification prototype. The primary challenge during development stemmed from the severe lack of documentation on the new C++ SDK interfaces, compounded by discrepancies in the official documentation's descriptions of C++ APIs. The current code still has significant room for optimization and improvement. Developers are welcome to engage in discussions, submit pull requests, and collaborate on iterative enhancements.
